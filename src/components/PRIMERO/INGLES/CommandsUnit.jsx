...
import ListenAct from "./COMMANDS/ListenAct";
import BuildCommand from "./COMMANDS/Buildcommand";
import TypeIt from "./COMMANDS/Typeit";

import "./CommandsUnit.css";
import confetti from "canvas-confetti";
import { guardarResultadoUnidad } from "../../../hooks/useGuardarResultado";
import { WorldBg, SPARKLES } from "../../Shared/SharedBackground";

const LEVELS = [
  { number: 1, title: "Listening", description: "Escucha la orden del rey y elige la imagen correcta", icon: "🎧", maxScore: 6 },
  { number: 2, title: "Reading", description: "Lee la frase y elige la imagen correcta", icon: "👁️", maxScore: 5 },
  { number: 3, title: "Writing", description: "Mira la imagen y completa la palabra", icon: "✏️", maxScore: 6 },
];
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#0A0015" }}>
      <div style={{ textAlign: "center", color: "#fff", fontFamily: "Nunito, sans-serif" }}>
        ...
        <button onClick={() => onFinish(0)} style={{ padding: "14px 32px", borderRadius: 50, border: "none", background: "#7C3AED", color: "#fff", fontFamily: "Nunito, sans-serif", fontWeight: 900, fontSize: 16, cursor: "pointer" }}>
          Back ←
        </button>
      </div>
    </div>
...
export default function CommandsUnit() {
  ...
  const handleSaveAndFinish = async (finalScores) => {
    ...
    try {
      ...
      await guardarResultadoUnidad({ unidad: "commands", periodo: 2, area: "ingles", niveles, totalScore, maxTotal });
    } catch (e) {
  ...
  if (phase === "intro") return (
    <div className="cu-screen">
      ...
      <div className="cu-card">
        ...
        <h1 className="cu-unit-title">Commands</h1>
        <p className="cu-unit-desc">Aprende a dar y seguir órdenes en inglés.</p>
  ...
  if (phase === "level") return (
    <div className="cu-level-wrapper">
      ...
      {currentLevel === 0 && <ListenAct onFinish={handleLevelFinish} />}
      {currentLevel === 1 && <BuildCommand onFinish={handleLevelFinish} />}
      {currentLevel === 2 && <TypeIt onFinish={handleLevelFinish} />}
    </div>
  );
  ...
  if (phase === "summary") return (
    <div className="cu-screen">
      ...
      <div className="cu-card cu-summary-card">
        <div className="cu-summary-emoji">{unitStars === 3 ? "👑" : unitStars === 2 ? "🏰" : "⚔️"}</div>
        <div className="cu-unit-badge">Commands · Completo</div>
