...
function FloatingPoints({ points, id }) {
    return <div key={id} className={`tl1-floating-points ${points > 0 ? "pos" : "neg"}`}>
        {points > 0 ? `+${points}` : points}
    </div>;
}

const NEON_SIGNS = [
    { x: 720, y: 90, text: "TOYS!", color: "#FF6B9D", glow: "rgba(255,107,157,0.9)", size: 56 },
];
...
const ToysBg = memo(function ToysBg({ color }) {
    return (
        <svg className="tl1-bg-svg" viewBox="0 0 1440 900" preserveAspectRatio="xMidYMid slice">
            <defs>
                <linearGradient id="tl1Sky" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#FFE4F1" />
                    <stop offset="40%" stopColor="#FFC8E0" />
                    <stop offset="100%" stopColor="#FFB3D1" />
                </linearGradient>
                <radialGradient id="tl1Glow" cx="50%" cy="45%" r="60%">
                    <stop offset="0%" stopColor={color} stopOpacity="0.28" />
                    <stop offset="100%" stopColor={color} stopOpacity="0" />
                </radialGradient>
                <linearGradient id="tl1Shelf" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#92400E" />
                    <stop offset="50%" stopColor="#B45309" />
                    <stop offset="100%" stopColor="#78350F" />
                </linearGradient>
                <linearGradient id="tl1Wood" x1="0" y1="0" x2="1" y2="0">
                    <stop offset="0%" stopColor="#7C2D12" />
                    <stop offset="100%" stopColor="#92400E" />
                </linearGradient>
                <linearGradient id="tl1Floor" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#78350F" stopOpacity="0.5" />
                    <stop offset="100%" stopColor="#451A03" stopOpacity="0.7" />
                </linearGradient>
                <symbol id="tl1Star" viewBox="-10 -10 20 20">
                    <path d="M0,-10 L3,-3 L10,-3 L4,2 L6,9 L0,5 L-6,9 L-4,2 L-10,-3 L-3,-3 Z" fill="currentColor" />
                </symbol>
                <symbol id="tl1Heart" viewBox="-12 -12 24 24">
                    <path d="M0,8 C-12,-2 -10,-12 -4,-12 C-1,-12 0,-9 0,-7 C0,-9 1,-12 4,-12 C10,-12 12,-2 0,8 Z" fill="currentColor" />
                </symbol>
                <filter id="tl1NeonGlow" x="-50%" y="-50%" width="200%" height="200%">
                ...
                <radialGradient id="tl1Vignette" cx="50%" cy="50%" r="75%">
                    <stop offset="60%" stopColor="#000" stopOpacity="0" />
                    <stop offset="100%" stopColor="#831843" stopOpacity="0.35" />
                </radialGradient>
            </defs>

            <rect width="1440" height="900" fill="url(#tl1Sky)" />
            ...
            <rect width="1440" height="900" fill="url(#tl1Glow)" />

            <rect y="780" width="1440" height="120" fill="url(#tl1Floor)" />
            ...
            <g opacity="0.85">
                <rect x="40" y="430" width="1360" height="14" fill="url(#tl1Shelf)" rx="2" />
                <rect x="40" y="430" width="14" height="180" fill="url(#tl1Wood)" />
                <rect x="1386" y="430" width="14" height="180" fill="url(#tl1Wood)" />
                <rect x="40" y="610" width="1360" height="14" fill="url(#tl1Shelf)" rx="2" />
            </g>
            ...
            {NEON_SIGNS.map((s, i) => (
                <g key={`neon${i}`} filter="url(#tl1NeonGlow)">
                    <text
                        x={s.x} y={s.y}
                        fontSize={s.size}
                        ...
                        fill={s.color}
                        textAnchor="middle"
                        style={{ animation: `tl1-neonFlicker 2.5s ease-in-out infinite alternate` }}
                    >
                        {s.text}
                    </text>
                </g>
            ...
            {SPARKLES.map((s, i) => (
                <circle key={`sp${i}`}
                    ...
                    style={{ animation: `tl1-sparkle ${s.dur} ${s.delay} ease-in-out infinite alternate` }} />
            ))}
            ...
            {STAR_SIGNS.map((s, i) => (
                <use key={`star${i}`}
                    ...
                    color={s.color}
                    style={{ animation: `tl1-starTwinkle ${s.dur} ${s.delay} ease-in-out infinite alternate` }} />
            ))}
            ...
            {HEART_DECOR.map((h, i) => (
                <use key={`heart${i}`}
                    ...
                    color={h.color}
                    style={{ animation: `tl1-heartFloat ${h.dur} ${h.delay} ease-in-out infinite alternate` }} />
            ))}
            ...
            {FLOATING_TOYS.map((t, i) => (
                <text key={`ft${i}`}
                    x={t.x} y={t.y}
                    fontSize={t.size}
                    style={{ animation: `tl1-toyFloat ${t.dur} ${t.delay} ease-in-out infinite alternate` }}>
                    {t.emoji}
                </text>
            ))}
                ...
                {Array.from({ length: 12 }, (_, i) => {
                    ...
                    return (
                        <polygon key={`bunting${i}`}
                            ...
                            style={{ animation: `tl1-bunting 2s ${i * 0.15}s ease-in-out infinite alternate` }}
                        />
                    );
                })}
            ...
            <rect width="1440" height="900" fill="url(#tl1Vignette)" />

            <circle cx="40" cy="40" r="14" fill="#FCD34D" opacity="0.7"
                style={{ animation: `tl1-bulbBlink 1.5s ease-in-out infinite alternate` }} />
            <circle cx="1400" cy="40" r="14" fill="#FF6B9D" opacity="0.7"
                style={{ animation: `tl1-bulbBlink 1.5s 0.5s ease-in-out infinite alternate` }} />
            <circle cx="40" cy="860" r="14" fill="#06D6A0" opacity="0.7"
                style={{ animation: `tl1-bulbBlink 1.5s 1s ease-in-out infinite alternate` }} />
            <circle cx="1400" cy="860" r="14" fill="#A78BFA" opacity="0.7"
                style={{ animation: `tl1-bulbBlink 1.5s 0.7s ease-in-out infinite alternate` }} />
        </svg>
    );
});

export default function ToysLevel1({ onFinish }) {
    ...
    useEffect(() => {
        ...
        const timeouts = timeoutRef.current;  // ← copia local del ref
        return () => {
            ...
            timeouts.forEach(clearTimeout);  // ← usa la copia
    ...
    const handleSelect = useCallback((option) => {
        ...
        if (option.word === currentToy.word) {
            ...
            setWrongCard(option.word);
            setShake(true);
            const tShake = setTimeout(() => setShake(false), 500);
            const tWrongCard = setTimeout(() => setWrongCard(null), 700);
            timeoutRef.current.push(tShake, tWrongCard);
            ...
            if (newLives <= 0) {
                setSolved(true);
                setRevealedCard(currentToy.word);
                setShowRetryBanner(false);
    ...
    }, [solved, finished, currentToy, errorsThisRound, streak, bestStreak, lives, current, triggerNext, playAudio]);
    ...
    if (finished) return (
        <div className="tl1-game-root tl1-result-container">
            <ToysBg color="#FF6B9D" />
            {floatingPoints.map(({ pts, id }) => <FloatingPoints key={id} points={pts} id={id} />)}
            <div className="tl1-result-card">
                <div className="tl1-result-emoji">🏆</div>
                <div className="tl1-result-badge">🧸 Toys · Level 1 🎧</div>
                <h2 className="tl1-result-title">¡Lo lograste!</h2>
                <div className="tl1-result-stars">
                    {Array.from({ length: 3 }).map((_, i) => (
                        <span key={i} className={`tl1-rstar ${i < unitStars ? "lit" : "dim"}`}>⭐</span>
                    ))}
                </div>
                <div className="tl1-result-stats">
                    <div className="tl1-rstat"><span>✅</span><span>Correct</span><strong>{score}/{questions.length}</strong></div>
                    <div className="tl1-rstat"><span>⚡</span><span>Points</span><strong>{totalPoints}</strong></div>
                    <div className="tl1-rstat"><span>🔥</span><span>Best streak</span><strong>{bestStreak}x</strong></div>
                    <div className="tl1-rstat"><span>🎯</span><span>Attempts</span><strong>{attempts}</strong></div>
                    <div className="tl1-rstat"><span>⏱</span><span>Time</span><strong>{timeElapsed}s</strong></div>
                </div>
                {onFinish && (
                    <button className="tl1-result-btn" onClick={() => onFinish(score)}>
                        Finalizar 🎉
                    </button>
                )}
            </div>
    ...
    return (
        <div className="tl1-game-root">
            <ToysBg color={roundColor} />
            {floatingPoints.map(({ pts, id }) => <FloatingPoints key={id} points={pts} id={id} />)}

            <div className="tl1-header-bar">
                <div className="tl1-header-left">
                    <span className="tl1-header-badge">Level 1</span>
                    <span className="tl1-header-title">🎧 Listening</span>
                </div>
                <div className="tl1-header-right">
                    <div className="tl1-header-pill">⚡ {totalPoints}</div>
                    {streak >= 2 && <div className="tl1-header-pill tl1-streak-pill">🔥 {streak}x</div>}
                    <div className="tl1-header-pill">🎯 {attempts}</div>
                    <div className="tl1-header-pill">⏱ {timeElapsed}s</div>
                </div>
            </div>

            <div className="tl1-listen-container">
                <div className={`tl1-wrapper ${shake ? "tl1-shake" : ""} ${bounce ? "tl1-bounce" : ""}`}>

                    <div className="tl1-progress-track">
                        <div className="tl1-progress-fill"
                            style={{
                                width: `${progress}%`,
                                background: `linear-gradient(90deg,${roundColor},#FCD34D)`,
                                boxShadow: `0 0 12px ${roundGlow}`
                            }} />
                        <div className="tl1-progress-steps">
                            {questions.map((_, i) => (
                                <div key={i} className={`tl1-progress-dot ${i < current ? "done" : i === current ? "active" : ""}`}
                                    style={i === current ? { background: roundColor, boxShadow: `0 0 8px ${roundColor}` } : {}} />
                            ))}
                        </div>
                    </div>

                    <p className="tl1-instruction">🛍️ Listen and pick the right toy!</p>

                    <div className="tl1-play-row">
                        <button
                            className={`tl1-play-btn ${isPlaying ? "playing" : ""} ${revealed ? "revealed" : ""}`}
                            onClick={playAudio}
                            disabled={isPlaying || solved}
                            style={{
                                "--round-color": roundColor,
                                "--round-glow": roundGlow,
                                background: `linear-gradient(135deg, ${roundColor}, #FCD34D)`,
                                boxShadow: `0 14px 40px ${roundGlow}, 0 0 32px ${roundGlow}`
                            }}
                            aria-label="Reproducir audio"
                        >
                            <span className="tl1-play-icon">{isPlaying ? "🔊" : "▶"}</span>
                            <span className="tl1-play-text">{isPlaying ? "Listen…" : "Play"}</span>
                        </button>

                        <div className="tl1-lives-container">
                            {Array.from({ length: MAX_LIVES }).map((_, i) => (
                                <span key={i} className={`tl1-heart ${i < lives ? "alive" : "lost"}`}>
                                    {i < lives ? "❤️" : "🖤"}
                                </span>
                            ))}
                        </div>
                    ...
                    <div className="tl1-banner-slot">
                        {showRetryBanner && (
                            <div className="tl1-retry-banner">❌ ¡Casi! 🛍️ Escucha de nuevo</div>
                        )}
                        {revealedCard && (
                            <div className="tl1-retry-banner found">✨ ¡Aquí está! Era "{currentToy.word}"</div>
                        )}
                    </div>

                    {/* OPTIONS GRID — IMÁGENES LIBRES SIN MARCO */}
                    <div className="tl1-options-grid">
                        {currentQuestion.options.map((option, i) => {
                            const isCorrect = correctCard === option.word;
                            const isWrong = wrongCard === option.word;
                            const isReveal = revealedCard === option.word;
                            return (
                                <button
                                    key={`${option.word}-${i}`}
                                    className={`tl1-toy-btn
                                        ${isCorrect ? "correct" : ""}
                                        ${isWrong ? "wrong" : ""}
                                    ...
                                    style={{
                                        ...
                                        "--card-glow": option.glow
                                    }}
                                >
                                    <img src={option.image} alt={option.word} className="tl1-toy-img" />
                                    <span className="tl1-toy-label">{option.word}</span>
                                </button>
                            );
                        })}
                    </div>
