...
import DragGame from "./GREETINGS/DragGame";
import ListenLevel from "./GREETINGS/ListenLevel";
import ConversationBuilder from "./GREETINGS/Conversationbuilder";
import "./GreetingsUnit.css";
import confetti from "canvas-confetti";
import { guardarResultadoUnidad } from "../../../hooks/useGuardarResultado";
import { WorldBg, SPARKLES } from "../../Shared/SharedBackground";

const LEVELS = [
  { number: 1, title: "Listening", description: "Escucha el audio y elige la imagen correcta", icon: "🎧", maxScore: 4 },
  { number: 2, title: "Reading", description: "Arrastra cada frase a la imagen correcta", icon: "👁️", maxScore: 4 },
  { number: 3, title: "Writing", description: "Escucha y escribe el saludo correcto", icon: "✏️", maxScore: 6 },
];

export default function GreetingsUnit() {
  ...
  const handleSaveAndFinish = async (finalScores) => {
    ...
    try {
      ...
      await guardarResultadoUnidad({ unidad: "greetings", periodo: 1, area: "ingles", niveles, totalScore, maxTotal });
    } catch (e) {
  ...
  if (phase === "intro") return (
    <div className="gu-screen">
      ...
      <div className="gu-card">
        ...
        <h1 className="gu-unit-title">Greetings</h1>
        <p className="gu-unit-desc">Aprende a saludar en inglés según el momento del día.</p>
  ...
  if (phase === "level") return (
    <div className="gu-level-wrapper">
      ...
      {currentLevel === 0 && <ListenLevel onFinish={handleLevelFinish} />}
      {currentLevel === 1 && <DragGame onFinish={handleLevelFinish} />}
      {currentLevel === 2 && <ConversationBuilder onFinish={handleLevelFinish} />}
    </div>
  );
  ...
  if (phase === "summary") return (
    <div className="gu-screen">
      ...
      <div className="gu-card gu-summary-card">
        ...
        <div className="gu-unit-badge">Greetings · Completo</div>
